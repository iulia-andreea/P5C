...
Gameplay: Chaque joueur connecté contrôle un serpent. Le serpent se déplace sa tête, et le reste le suit, dans la direction 
du dernier clic. Quand un serpent percute un autre serpent ou un obstacle, il est réinitialisé à une position
aléatoire.
...
# GÉNÉRATION DU SSL CLE ET DU CERTIFICAT
Pour générer une clé et un certificat pour l'exécution d'un serveur HTTPS, vous devez exécuter la commande suivante dans 
un terminal:
-> openssl req -nodes -new -x509 -keyout server.key -out server.cert

Ensuite, vous devez déplacer les fichiers du dossier dans lequel ils ont été créés dans le dossier "~/P5C/priate".
...
- depuis un autre ordinateur personnel:
    ...
    //où <ip_server> est l'adresse IP IPv4 de la machine qui exécute le fichier du serveur,
    //et 8081 est le port utilisé par le serveur https (le serveur écoute a cet port).
